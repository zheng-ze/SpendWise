...
Standard, already-common acronyms (DB, API, ID) are fine. Don't shorten a real word into a
made-up one (`cfg`, `impl`) to save space — a tokenizer splits it into the same pieces as the full
word, so nothing is saved and the finding gets harder to read.
