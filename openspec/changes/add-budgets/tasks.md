...
- [x] 1.1 Add `YearMonth` to `packages/domain/lib/src/calendar_day.dart` (or a new
...
- [x] 1.2 Test comparison, equality, and `fromUtc` in a new `year_month_test.dart`, including that
      December-to-January comparisons order correctly across a year boundary.

## 2. LimitEvent and Budget models

- [x] 2.1 Add `packages/domain/lib/src/limit_event.dart`: `LimitEvent {effectiveFromMonth:
...
- [x] 2.2 Add `packages/domain/lib/src/budget.dart`: `Budget {id, categoryID: String?,
      ...
      `==`/`hashCode`.
- [x] 2.3 Add a pure `effectiveLimit(Budget budget, YearMonth month)` function (design.md's
...
- [x] 2.4 Test `effectiveLimit` directly against `LimitEvent` lists (no `LedgerState` involved):
...
- [x] 3.1 Add `UpsertBudget` and `DeleteBudget` to `ledger_change.dart`, following the existing
      `UpsertPlan`/`DeletePlan` shape (`targetID`, value `==`, `hashCode`, `toString`).
- [x] 3.2 Add new `LedgerError` cases to `ledger_error.dart` for: unknown budget id (thrown on
      ...
      is enough since all three are "this carryCap value is unusable"). Non-positive amount reuses
      the existing `ZeroAmount` case rather than a new one, since the task list names only these
      three as new and entries have no equivalent "non-positive" case to depart from.
- [x] 3.3 Test each new error's `==`/`hashCode`/`toString` alongside the existing cases in
      `ledger_change_test.dart` / wherever those sealed-class tests live, matching the existing
      per-case test shape.

## 4. Budget mutators

- [x] 4.1 Add `packages/domain/lib/src/ledger_state_budgets.dart` as a `part of 'ledger_state.dart'`
      extension `LedgerStateBudgets`, mirroring `ledger_state_plans.dart`'s structure.
- [x] 4.2 Implement `addBudget(categoryID, initialAmount, rolloverMode, {carryCap})`: validates
...
- [x] 4.3 Implement `updateBudgetAmount(budgetID, newAmount, effectiveFromMonth)`: looks up the
...
- [x] 4.4 Implement `setBudgetMonthOverride(budgetID, month, value)`: looks up the budget, validates
      `value > 0` and `month` non-null, appends `{effectiveFromMonth: month, value, kind:
      override}`, returns `_checked([UpsertBudget(updated)])`.
- [x] 4.5 Implement `deleteBudget(budgetID)`: hard-removes the budget, no-op on a missing id,
      matching `deletePlan`'s shape exactly.
- [x] 4.6 Test each mutator directly against `LedgerState`: creation success and every rejection
...
- [x] 5.1 Add `_removeBudgetCategorized(String categoryID)` to `ledger_state_budgets.dart`,
...
- [x] 5.2 Call `_removeBudgetCategorized(category.id)` alongside the existing
      `_removePlansCategorized(category.id)` call in `_purgeCategoryRow`
      (`ledger_state_purge.dart:91`).
- [x] 5.3 Call `_removeBudgetCategorized(categoryID)` alongside the existing
      `_removePlansCategorized(categoryID)` call in `_sweepCategory` (`ledger_state_purge.dart:134`).
- [x] 5.4 Test the cascade: deleting a category with a budget removes the budget and emits
...
- [x] 6.1 Read `ledger_state_invariants.dart` to find where categorized-row invariants
...
- [x] 6.2 Test the invariant catches a deliberately malformed state the normal mutator path cannot
...
- [x] 7.1 Run `cd packages/domain && dart format . && dart analyze && dart test` and confirm zero
      analyzer issues and all tests green. `packages/domain` gate: zero issues, 585/585 tests
      green. `flutter analyze` in `app/`: one known, deliberately unfixed issue —
      `drift_ledger_store.dart`'s `_apply` switch is not exhaustive over `UpsertBudget`/
      `DeleteBudget`. Left failing on purpose: budgets have no storage table yet, and a case that
      silently no-ops (even logged) would make the build go green while hiding that persistence is
      still missing. The next session should see this red and know real work remains, not a
      quiet-logged stand-in. `error_section.dart`'s `friendlyLedgerErrorMessage` got the three new
      `LedgerError` cases with real user-facing messages, since that handling is permanent and not
      a stand-in for anything.
