...
void main() {
  ...
  group('buildRecurringPlanForNewEntry', () {
    ...
    test('sets the cursor one calendar day behind the anchor', () {
      ...
      expect(plan.lastResolvedDate, DateTime.utc(2026, 8, 14));
      expect(plan.anchor, anchor);
    });

    test('the anchor day itself resolves immediately from that cursor', () {
      final anchor = DateTime.utc(2026, 8, 15);
      final plan = buildRecurringPlanForNewEntry(
        template: template,
        frequency: RecurrenceFrequency.monthly,
      ...
      final occurrences = plan.occurrences(
        after: plan.lastResolvedDate,
        upTo: anchor,
      );
      expect(occurrences, [anchor]);
    });

    test('the seed convention of cursor-at-anchor would skip it', () {
      final anchor = DateTime.utc(2026, 8, 15);
      final seedStylePlan = RecurringPlan(
        template: template,
        frequency: RecurrenceFrequency.monthly,
        anchor: anchor,
      ...
      final occurrences = seedStylePlan.occurrences(
        after: seedStylePlan.lastResolvedDate,
        upTo: anchor,
      );
      expect(occurrences, isEmpty);
    });

    test('carries the optional end date through', () {
      final anchor = DateTime.utc(2026, 8, 15);
      final end = DateTime.utc(2026, 12, 1);
      final plan = buildRecurringPlanForNewEntry(
        template: template,
        frequency: RecurrenceFrequency.monthly,
        anchor: anchor,
